...
export const Store: React.FC<StoreProps> = ({ 
  ...
  const confirmPurchase = () => {
      ...
      let success = false;
      if (selectedProduct.currency === 'STARS' && selectedProduct.rewardAmount) {
          success = onExchangeStars(selectedProduct.price, selectedProduct.rewardAmount);
      } else {
          // Fix: Pass categoryToEquip to handle auto-equip in the same transaction
      ...
      if (success) {
          ...
          // Removed manual onEquip call here as it's now handled by onPurchase to prevent race conditions
      } else {
          playStoreSound('ERROR');
          const currencyName = selectedProduct.currency === 'STARS' ? 'Estrelas' : 'PoeiraCoins';
          showFeedback(`${currencyName} insuficientes!`, 'error');
      }
  ...
  const handleAdClick = () => {
      ...
      setTimeout(() => {
          onWatchAd();
          showFeedback("+5 PoeiraCoins!", 'success');
          setAdLoading(false);
          playStoreSound('BUY');
      }, 1500);
  };
  ...
  return (
    <div className="w-full h-full bg-[#fdf6e3] font-sans flex flex-col relative overflow-hidden select-none">
        ...
        {/* Feedback Toast */}
        {feedback && (
            <div className={`absolute top-24 left-1/2 transform -translate-x-1/2 z-[60] px-6 py-3 rounded-full font-black text-white shadow-xl animate-bounce ${feedback.type === 'success' ? 'bg-green-500' : 'bg-red-500'}`}>
                {feedback.msg}
            </div>
        )}
            ...
            <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4">
                <div className="bg-white rounded-3xl border-4 border-slate-800 shadow-[0_10px_0_rgba(0,0,0,0.3)] w-full max-w-sm overflow-hidden transform scale-100 transition-all">
                    <div className="bg-slate-100 p-6 flex flex-col items-center border-b-2 border-slate-200">
                        ...
                        <div className="w-24 h-24 mb-4 flex items-center justify-center animate-bounce-slow">
                    ...
                    <div className="p-6">
                        <div className="flex justify-between items-center mb-6 bg-slate-50 p-3 rounded-xl border border-slate-200">
                            ...
                            <div className="flex items-center text-2xl font-black">
                                {selectedProduct.currency === 'STARS' ? <Star size={24} className="text-yellow-400 fill-yellow-400 mr-2"/> : <span className="mr-2">🪙</span>}
                                <span className={selectedProduct.currency === 'STARS' ? 'text-yellow-600' : 'text-slate-800'}>{selectedProduct.price}</span>
                            </div>
                        </div>

                        <div className="flex gap-3">
                            <button onClick={() => setSelectedProduct(null)} className="flex-1 py-3 rounded-xl font-black text-slate-500 border-b-4 border-slate-300 bg-slate-200 hover:bg-slate-300 active:border-b-0 active:translate-y-1 transition-all">
                                CANCELAR
                            </button>
                            <button onClick={confirmPurchase} className="flex-1 py-3 rounded-xl font-black text-white border-b-4 border-green-700 bg-green-500 hover:bg-green-400 active:border-b-0 active:translate-y-1 transition-all shadow-lg flex items-center justify-center">
                                <ShoppingCart size={20} className="mr-2"/> COMPRAR
                            </button>
                        </div>
                    </div>
                </div>
        ...
        {/* --- CABEÇALHO --- */}
        <div className="h-24 bg-white border-b-8 border-orange-200 flex items-center justify-between px-6 z-20 shadow-sm relative">
             <div className="absolute top-0 left-0 right-0 h-4 w-full flex" style={{ top: '100%' }}>
                {Array.from({ length: 20 }).map((_, i) => (
                    <div key={i} className={`flex-1 h-4 rounded-b-lg ${i % 2 === 0 ? 'bg-red-400' : 'bg-white'}`}></div>
                ))}
             </div>

             <div className="flex items-center">
                <button onClick={onBack} className="p-3 bg-orange-500 hover:bg-orange-400 rounded-2xl mr-4 text-white transition-all active:scale-95 border-b-4 border-orange-700 active:border-b-0 active:translate-y-1 shadow-lg">
                    <ArrowLeft size={28} strokeWidth={3} />
                </button>
                <div className="flex flex-col">
                    <h2 className="text-2xl md:text-3xl font-black text-orange-500 italic tracking-tighter drop-shadow-sm leading-none">
                        LOJA DA CASA MÁGICA
                    </h2>
             ...
             <div className="flex items-center gap-3">
                 {activeTab === 'PACOTES' && (
                     <div className="flex items-center bg-slate-800 px-3 py-1 rounded-full border-2 border-slate-600 animate-fade-in">
                 ...
                 <button 
                    ...
                    disabled={adLoading}
                    className={`flex items-center bg-purple-600 px-3 py-2 rounded-xl border-b-4 border-purple-800 active:border-b-0 active:translate-y-1 transition-all text-white ${adLoading ? 'opacity-70 cursor-wait' : 'hover:bg-purple-500 shadow-lg'}`}
                 >
                     <div className="mr-2 animate-pulse">
                         {adLoading ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <Tv size={20} />}
                     </div>
                     <div className="flex flex-col leading-none items-start">
                         <span className="text-[10px] font-bold opacity-80 uppercase">Comercial</span>
                         <span className="text-sm font-black">+5 🪙</span>
                     </div>
                 </button>

                 <div className="flex items-center bg-yellow-400 px-4 py-2 rounded-full border-4 border-yellow-500 shadow-inner transform rotate-1">
                     <div className="w-10 h-10 bg-yellow-300 rounded-full border-2 border-yellow-600 flex items-center justify-center mr-2 shadow-sm">
                        <span className="text-2xl">🪙</span>
                     </div>
                     <div className="flex flex-col items-end leading-none">
                         <span className="text-2xl font-black text-yellow-900">{poeiraCoins}</span>
                         <span className="text-[10px] font-bold text-yellow-800 uppercase">PoeiraCoins</span>
                     </div>
                 </div>
             </div>
        </div>
        ...
        {/* --- ABAS --- */}
        <div className="flex justify-center space-x-2 mt-8 px-4 z-10 relative overflow-x-auto pb-2 md:pb-0">
            <TabButton label="Skins" icon={<Palette size={20}/>} active={activeTab === 'SKINS'} onClick={() => { setActiveTab('SKINS'); playStoreSound('SELECT'); }} color="bg-rose-500" />
            <TabButton label="Efeitos" icon={<Sparkles size={20}/>} active={activeTab === 'EFEITOS'} onClick={() => { setActiveTab('EFEITOS'); playStoreSound('SELECT'); }} color="bg-violet-500" />
            <TabButton label="Pacotes" icon={<Package size={20}/>} active={activeTab === 'PACOTES'} onClick={() => { setActiveTab('PACOTES'); playStoreSound('SELECT'); }} color="bg-amber-500" />
            <TabButton label="Extras" icon={<Gift size={20}/>} active={activeTab === 'EXTRAS'} onClick={() => { setActiveTab('EXTRAS'); playStoreSound('SELECT'); }} color="bg-emerald-500" />
        </div>

        {/* --- ÁREA PRINCIPAL (PRATELEIRAS) --- */}
        <div className="flex-1 overflow-y-auto p-4 md:p-8 relative z-0 mt-2 custom-scrollbar">
            <div className="absolute inset-0 pointer-events-none opacity-50" style={{ backgroundImage: 'linear-gradient(transparent 95%, rgba(0,0,0,0.1) 95%)', backgroundSize: '100% 160px' }}></div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pb-20">
                {items[activeTab].map((product) => {
                    const isOwned = ownedItems.includes(product.id);
                    // Verificação de equipamento robusta para slots separados
                    ...
                    let btnClass = isStarCost ? "bg-blue-600 border-blue-800 text-white hover:bg-blue-500" : "bg-green-500 border-green-700 text-white hover:bg-green-400";
                    let isDisabled = false;

                    if (!isConsumable && isOwned) {
                        if (isEquipped) {
                            btnText = "EQUIPADO";
                            btnIcon = <Check size={16} className="mr-1" />;
                            btnClass = "bg-yellow-400 border-yellow-600 text-yellow-900 opacity-100 cursor-default";
                            isDisabled = true;
                        } else {
                            btnText = "EQUIPAR";
                            btnIcon = <Shirt size={16} className="mr-1" />;
                            btnClass = "bg-cyan-500 border-cyan-700 text-white hover:bg-cyan-400";
                        }
                    }

                    return (
                        <div key={product.id} className="relative group">
                            <div className={`${product.color} h-64 rounded-3xl border-4 border-white shadow-xl flex flex-col items-center p-4 transition-transform hover:-translate-y-2 hover:rotate-1 relative overflow-hidden`}>
                                <div className="absolute inset-0 bg-white opacity-0 group-hover:opacity-20 transition-opacity"></div>
                                
                                {isOwned && !isConsumable && (
                                    <div className="absolute top-2 right-2 bg-green-500 text-white p-1 rounded-full shadow-md z-10 animate-bounce-slow">
                                ...
                                <div className="w-full text-center mb-2">
                                    <h3 className={`font-black text-lg leading-tight ${[1, 3].includes(product.id) ? 'text-white' : 'text-slate-800'}`}>{product.name}</h3>
                                    <p className={`text-[10px] font-bold opacity-70 leading-tight mt-1 ${[1, 3].includes(product.id) ? 'text-white' : 'text-slate-600'}`}>{product.description}</p>
                                </div>

                                <button 
                                    onClick={() => handleProductClick(product)}
                                    disabled={isDisabled}
                                    className={`w-full py-2 rounded-xl font-black text-sm shadow-md border-b-4 transition-all active:border-b-0 active:translate-y-1 flex items-center justify-center ${btnClass}`}
                                >
                                    {btnIcon}
                                    {isOwned && !isConsumable && !isEquipped ? btnText : (isEquipped ? btnText : (
                                        <span>{product.price}</span>
                                    ))}
                                </button>
                            </div>
                            <div className="absolute -bottom-4 left-4 right-4 h-2 bg-black/20 rounded-full blur-sm"></div>
                        </div>
                    );
                })}
            </div>
...
const TabButton = ({ label, icon, active, onClick, color }: any) => (
    <button 
        onClick={onClick}
        className={`
            flex items-center px-4 py-3 rounded-t-2xl font-black text-sm md:text-base transition-all whitespace-nowrap
            ${active 
                ? `${color} text-white shadow-lg transform -translate-y-1` 
